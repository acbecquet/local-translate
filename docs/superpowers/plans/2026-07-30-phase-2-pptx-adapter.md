...
4. Table: cell w from `a:gridCol` widths, h from `a:tr` heights; horizontal merge (gridSpan) unions column widths; vertical merge (vMerge) unions row heights.
   > **Plan amendment (Phase 2 whole-phase review, finding 4):** the union box also has the anchor cell's own `a:tcPr` margins (explicit `marL`/`marR`/`marT`/`marB`, else the OOXML defaults - 91440 EMU L/R, 45720 EMU T/B, same values as bodyPr's text insets) subtracted exactly once, mirroring how bodyPr insets are subtracted from a shape's box. A merged region's covered (hMerge/vMerge-only) cells are never consulted for their own tcPr - only the anchor's margins apply, since only the anchor carries real formatting per OOXML merge semantics.
...
1. For each translated segment: locate the same node by id; write the translation's lines... precisely: paragraphs of the translation (split on `\n`) map onto existing `a:p` elements 1:1 when counts match; when counts differ, all text goes into the first paragraph's first run and surplus paragraphs' runs are emptied (formatting elements retained). First run keeps its `a:rPr`; sibling runs in a replaced paragraph are emptied, not deleted.
   > **Sanctioned deviation #1 (Phase 1 final review, carried into the Phase 2 whole-phase review as finding 7):** surplus paragraphs are DELETED outright, not "emptied, not deleted" as stated above. An emptied-but-retained surplus paragraph is a phantom blank line - PowerPoint still renders it as real vertical space the fit engine never measured against, and a subsequent extract() would see an extra blank line the translation never asked for, breaking the round trip. Deleting the surplus paragraph does lose its own per-paragraph formatting (bullet level, alignment, spacing) - accepted as the lesser cost. See writeTranslation's own doc comment in pptx-adapter.ts. (Finding 2 of the same review extends this same line-capacity accounting to `a:br` groups WITHIN a paragraph, not just whole paragraphs - see paragraphGroups/writeLineIntoGroup.)
2. Font size: set `sz` (hundredths, floored to the quarter point - see sanctioned deviation #2's neighbor below) on every non-empty run in the body to the fitted size ONLY when fitted size differs from original; autofit elements (`a:normAutofit fontScale/lnSpcReduction`) are removed when we set an explicit size (we are the autofit now).
   > **Sanctioned deviation #2 (Phase 2 whole-phase review, finding 7):** autofit elements are NOT simply "removed" as stated above - a bare `<a:normAutofit/>` (no `fontScale`/`lnSpcReduction`, which would be stale values from a previous autofit computed for different text/size) is written instead, as a belt-and-suspenders safety net on top of our own explicit `sz` (research-adopted from LinguaHaru's approach, credited in ensureNormAutofit's doc comment). This also amends line 2's "rounded to nearest 25" to FLOORED to the quarter point: rounding to the nearest quarter point can round UP past what fit() actually measured as fitting, silently reintroducing overflow; flooring only ever writes a size at or below the measured size.
